...
rem
rem	1.4.0	13-11-2013	* se agrego en 1er intento de recuperacion eliminar los archivos consulta* y libreta*
rem
...
@erase %origen%\out* /Q
@erase %origen%\libreta.* /Q
@erase %origen%\consulta.* /Q
@erase %origen%\chk_tmp*
@erase %origen%\ultarj* /Q
@erase t*.txt
...
:P4-99
:--
rem Out con error desconocido  
@erase t*.txt /Q
set paso=P4-99
set npaso=SALIR
set appl=MOS-CDS
set remote="error desconocido"
set pdesc=Depuracion de todos los idx en proceso - %remote%
@@title %PROG_ID% %pdesc%
cls
@echo --
@echo -- %pdesc%
...
set codigo=000
set escalar=0000
set desc="Depuracion de todos los idx en proceso% - Error desconocido"
@now.exe " %srv% | %ticket% | Appl:%appl% | Paso:%paso%-%pdesc% | %codigo% | %desc% | " >> %LOGF%
@erase %origen%\NOT_RUN /Q
@erase %origen%\ddtime /Q
