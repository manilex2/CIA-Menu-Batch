...
rem             en la cual si existe en el servidor las carpetas de soft impref y mos_new_rt se copian 
...
set remote=moa
@FINDSTR /I /C:"moa " net.txt 
IF NOT %ERRORLEVEL% == 0 GOTO RT-101
set remote=Imagen
@FINDSTR /I /C:"Imagen" net.txt 
IF NOT %ERRORLEVEL% == 0 GOTO RT-101
set remote=Imagen2
@FINDSTR /I /C:"Imagen2" net.txt 
IF NOT %ERRORLEVEL% == 0 GOTO RT-101
...
rem 
IF EXIST c:\windows\Microsoft.NET\Framework\v2.0* IF EXIST %soft%\MOS_NEW_RT\NEW_RT\rt.exe  @copy %soft%\MOS_NEW_RT\NEW_RT\* c:\moa\bin\* /Y
...
IF EXIST c:\"Documents and Settings" @copy %soft%\MOS_NEW_RT\NTUSER.DAT c:\"Documents and Settings\All Users"\ * /Y
...
set tfile=%tdicc%.log
if exist \\%moasrv%\moaproj\log\%tdicc%.log  goto rt
@FINDSTR /I /C:"%tdicc%" \\%moasrv%\moaproj\log\%logf%
...
@now.exe %PROG_ID% %origen% %tid% %MSG% >> \\%moasrv%\moaproj\log\%logf%
@now.exe %PROG_ID% %origen% %tid% %MSG% > c:\moaproj\%tproj%\dir_tmp.txt
@dir c:\moaproj\%tproj%\dd* >> c:\moaproj\%tproj%\dir_tmp.txt
@copy c:\moaproj\%tproj%\dir_tmp.txt \\%moasrv%\moaproj\log\%tfile% 
...
cls
@echo ** 
@echo ** CODIGO ERROR: RT-102 Usuario no tiene acceso a carpetas compartidas MOA o MOAPROJ
