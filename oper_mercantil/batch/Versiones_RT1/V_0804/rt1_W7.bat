@echo off
rem *********************************************************************
rem Archivo: rt1.bat ï  
rem
rem Version 0804 Oct 2013
rem
rem  Descripcion: 
...
set cont=N
set prot=IP
set soft=\\%srv%\oficina-mercantil\soft\drivers_y_aplicaciones
set terror=0
...
rem 
@FINDSTR /C:"ACTIVO=S" %soft%\MOS-NEW-RT\mos-new-rt.ini
IF NOT %ERRORLEVEL% == 0 GOTO MOSCAC
@copy %soft%\MOS-NEW-RT\RT\rt.exe c:\moa\bin\* /Y

:MOS-NEW-RT
rem 
rem Verificar version OS de la estacion
rem 
@copy %soft%\MOS-NEW-RT\NEW-RT\* c:\moa\bin\* /Y
@ver > twin.txt
@FINDSTR /C:" 6." twin.txt
IF %ERRORLEVEL% == 0 @copy c:\moa\bin\rt-w7.exe  rt.exe  /Y

rem **********************************************************************************************************

:MOSCAC
:------
@title %rtver% - Revisar si modalidad CAC esta activa
@erase q1.txt /Q
...
:PING-MOSPROD
:------------
@title %rtver% - Ejecutando ping a MOSPRODIP
@ping -n 3 MOSPRODIP
